...
def user_page(name):
    ...
    # bottle.TEMPLATES.clear()
    return bottle.template('user', postlist=postlist, userlist=user_list(),
        page='user', username=tuser['_id'], logged=True,
        is_following=tuser['_id'] in luser['followee'], himself=himself)
...
def status(name, id):
    ...
    return bottle.template('single', username=post['uid'], tweet_id=id,
            tweet_text=post['content'], page='single', logged=(session != None))
...
def static_file(filename):
    bottle.send_file(filename, root=os.path.join(
        os.environ['OPENSHIFT_REPO_DIR'], 'wsgi', 'static'))
